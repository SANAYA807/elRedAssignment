...
function VurtuallyMet() {
    ...
    const getData = async () => {
        const data = await axios.get('https://newpublicbucket.s3.us-east-2.amazonaws.com/reactLiveAssignment/JsonFiles/RatingsVirtuallyMetResponse.json')
    ...
    return (
        <div style={{ background: 'white', marginTop: '50px', padding: '20px', borderRadius: '20px', width: '375px', margin: 'auto' }}>
            <p style={{ color: '#bcbcbc', fontSize: '13px' }}><span style={{ fontSize: '15px', color: 'black' }}>{users.length}</span> have met in real life / video call</p>
